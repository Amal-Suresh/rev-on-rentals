...
const viewBikes = async (req, res) => {
    try {
        ...
        const totalItems = await Bike.countDocuments({ partnerId: req.id });
...
const findBookings = async (req, res) => {
    try {
        ...
        const limit = 5;
        const skip = (page - 1) * limit;
        const totalItems = await Booking.countDocuments({ partner: req.id });
        const totalPages = Math.ceil(totalItems / limit);
        const rides = await Booking.find({ partner: req.id }).populate('bike').populate('user').skip(skip).limit(limit)
        res.status(200).send({ success: true, message: "data featched successfully", data: rides ,page:page ,totalPages:totalPages })
    } catch (error) {
        console.log(error.message);
        res.status(401).send({ success: false, message: "something went wrong" })
    }
